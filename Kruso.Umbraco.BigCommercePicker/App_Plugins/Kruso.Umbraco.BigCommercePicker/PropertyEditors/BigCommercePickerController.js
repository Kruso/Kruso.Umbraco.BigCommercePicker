...
/**
 * The controller that is used for picking content,
 ...
 */
function bigCommercePickerController($scope, $q, $routeParams, bigCommerceResource, editorState, localizationService, editorService, overlayService, umbRequestHelper) {
    ...
    function validate() {
        if ($scope.bigCommercePickerForm) {
            //Validate!
            if ($scope.model.config && $scope.model.config.minNumber && parseInt($scope.model.config.minNumber) > $scope.renderModel.length) {
                $scope.bigCommercePickerForm.minCount.$setValidity("minCount", false);
            }
            else {
                $scope.bigCommercePickerForm.minCount.$setValidity("minCount", true);
            }

            if ($scope.model.config && $scope.model.config.maxNumber && parseInt($scope.model.config.maxNumber) < $scope.renderModel.length) {
                $scope.bigCommercePickerForm.maxCount.$setValidity("maxCount", false);
            }
            else {
                $scope.bigCommercePickerForm.maxCount.$setValidity("maxCount", true);
            }
        }
    }
    ...
    $scope.openCurrentPicker = function () {
        ...
        $scope.currentPicker.view = umbRequestHelper.convertVirtualToAbsolutePath("~/App_Plugins/Kruso.Umbraco.BigCommercePicker/Overlays/BigCommercePickerOverlay.html"),
    ...
    function setDirty() {
        if ($scope.bigCommercePickerForm && $scope.bigCommercePickerForm.modelValue) {
            $scope.bigCommercePickerForm.modelValue.$setDirty();
        }
    }
    ...
    function syncRenderModel(doValidation) {
        ...
        if (valueIds.length > 0) {
            ...
            if (missingIds.length > 0) {
                return bigCommerceResource.getByIds(missingIds, entityType, $routeParams.cculture ? $routeParams.cculture : $routeParams.mculture).then(function (data) {
    ...
    function init() {
        ...
        localizationService.localizeMany(["general_recycleBin", "general_add", "bigCommerce_select" + entityType + ($scope.model.config.multiPicker ? "Multi" : "")])
            .then(function (data) {
...
angular.module('umbraco').controller("Kruso.Umbraco.BigCommercePicker.PropertyEditors.BigCommercePickerController", bigCommercePickerController);